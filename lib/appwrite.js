...
export const getSavedUserPosts = async (userId) => {
    try {
        if (userId) {
            const posts = await databases.listDocuments(databaseId, userCollectionId, [Query.equal('$id', userId)]);

            return posts.documents[0].savedVideos;
        }
    } catch (error) {
...
export const saveUnsaveVideo = async ({ userId, videoId }) => {
    try {
        const result = await databases.updateDocument(databaseId, userCollectionId, userId);
    } catch (error) {
